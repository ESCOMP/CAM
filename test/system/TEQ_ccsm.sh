...
# Compare initial files
files_to_compare=`cd ${CAM_TESTDIR}/TSM_ccsm.$1.$2.$5; ls *.cam*.i*.nc`
if [ -z "${files_to_compare}" ]; then
    echo "FAIL.job${JOBID}" > TestStatus
    exit 6
fi
...
for compare_file in ${files_to_compare}; do
    ...
    mystring=${compare_file##*i.[0-9][0-9][0-9][0-9]}
    corresponding_file=`ls ${CAM_TESTDIR}/TSM.$3.$4.$5/*cam.i*${mystring}*`
