...
!!$   call budget_add('param_mpas' ,'dAM','dBF',pkgtype='dyn',optype='dif',longname="dE/dt physics total in dycore (phys)           (dAM-dBF)",outfld=.true.)
...
!jt   call print_budgets(dyn_in%nCellsSolve,dyn_out)

end subroutine dyn_run

subroutine print_budgets(nCells,dyn_out)
  ...
  integer          :: i
  character(len=*), parameter :: subname = 'dyn_comp:print_budgets'
  ...
  do b_ind=1,budget_num
     ...
     if (budget_pkgtype=='dyn') then
        if (budget_optype=='stg') then
           ...
           write(iulog,*)'update calculating te_budgets diff/sum for ',budget_name,' cnt=',budgets_cnt(b_ind),' b_ind=',b_ind,' s_ind=',s_ind
...
!jt  budgets_cnt     => dyn_out % budgets_cnt
  ...
  do b_ind=1,budget_num
     ...
     if (budget_pkgtype=='dyn') then
        ...
        write(iulog,*)'calculating area for ',budget_name,' cnts=',budgets_cnt(b_ind),' b_ind=',b_ind,' s_ind=',s_ind
        ...
        write(iulog,*)'setting budgets_cnt(b_ind) to zero for ',budget_name,' b_ind=',b_ind
     end if
  end do
