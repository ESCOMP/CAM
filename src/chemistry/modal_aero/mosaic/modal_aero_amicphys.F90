...
! true (or false) - mw_aer_host set from adv_mass ( or specmw_amode )
 ...
 integer :: igas_soa2, igas_soa3, igas_soa4, igas_soa5 ! zlu soa
  ...
  integer :: iaer_bc, iaer_dst, iaer_ncl, iaer_nh4, iaer_pom, iaer_soa, iaer_so4, &
             iaer_soa2, iaer_soa3, iaer_soa4, iaer_soa5, &  !zlu soa 
...
! get saturation mixing ratio
      ...
      ! MOSAIC (dsj) - It looks like qsat routine was changed between
      !                CESM2.1 and CAM6.3.018
      do k = 1, pver
         call qsat( t(1:ncol,k), pmid(1:ncol,k), &
                    ev_sat(1:ncol,k), qv_sat(1:ncol,k), ncol )      
      end do
...
! increment column tendencies
      ...
      do l = 1, nbin_a_max
         ...
         pH_fields(i,k,l) = MIN(14.0_r8, MAX(-3.0_r8, -log10(Hconc_sav(l)) ))
      end do
...
!              uptkrate_h2so4,    misc_vars_aa_sub                        )
        ...
        use module_data_mosaic_aero,   only: naer_mosaic => naer, &
             ...
             jhyst_up, jtotal, &
             ! move nbin_a_max to module use variable
...
!--------------------------------------------------------------------------------
      ...
      dgas = (1.0e-3_8 * t_k**1.75_8 * sqrt(1._r8/rmw + 0.035_8))/   &
...
!zlu soa
      ...
      else if (nsoa == 5) then            ! zlu soa for VBS option
         jsoa =      1 ; name_gas(jsoa) = 'SOAG0' ; name_aerpfx(jsoa) = 'soa1'! jsoa=1
         jsoa = jsoa+1 ; name_gas(jsoa) = 'SOAG1' ; name_aerpfx(jsoa) = 'soa2'! jsoa=2
         jsoa = jsoa+1 ; name_gas(jsoa) = 'SOAG2' ; name_aerpfx(jsoa) = 'soa3'! jsoa=3
         jsoa = jsoa+1 ; name_gas(jsoa) = 'SOAG3' ; name_aerpfx(jsoa) = 'soa4'! jsoa=4
         jsoa = jsoa+1 ; name_gas(jsoa) = 'SOAG4' ; name_aerpfx(jsoa) = 'soa5'! jsoa=5
 !     else if (nsoa == 6) then
...
!--------------------------------------------------------------------------------
      ...
      do iaer = 1, naer
         if      (iaer == iaer_so4) then
            ...
            i = ilim2_a
else if (iaer == iaer_soa2) then   ! zlu soa
            i = ilim1_a
