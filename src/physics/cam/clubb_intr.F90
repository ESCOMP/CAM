   ...
   if (do_clubb_mf) then
    ...
    do t = 1, nadv    ! do needed number of "sub" timesteps for each CAM step
      ...
      if (do_clubb_mf) then
        ...
        ! --- DIAGNOSTIC TRAP: Surface Boundary Array Bounds ---
        if (get_nstep() == 1 .and. i == 1 .and. t == 1) then
           write(iulog,*) 'DIAG_TRAP: mf_precc TOA = ', mf_precc(1,1)
           write(iulog,*) 'DIAG_TRAP: mf_precc SFC = ', mf_precc(1,nzm_clubb)
           write(iulog,*) 'DIAG_TRAP: th_sfc       = ', th_sfc(1)
        end if
        ! --- END TRAP ---
      end if
