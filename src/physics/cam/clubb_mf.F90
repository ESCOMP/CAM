...
!
       ...
       enddo
       !jt thl_env, qt_env, and thv_env were created as separate arrays was so the solver could extrapolate a subsurface "dummy" value (thl_env(1))
       !jt using the gradients without overwriting the actual atmospheric thl state.
       !jt However, the solver only uses that dummy value at the surface interface, where the downdraft velocity (awdn) is exactly 0.0.
       !jt This means the extrapolated dummy value is always mathematically multiplied by zero!
       !jt we can safely delete thl_env, thv_env, qt_env, betathl, and betaqt and cap the kt_up and kt_dn indices inside the flux loop so they never exceed 1 to nzt.
       !jt When the solver hits the zero-flux boundary, it will read a valid atmospheric cell, multiply it by 0.0, and cleanly yield 0.0.
!!$       ! --------------------------------------------------------- !
!!$       ! get turbulent fluxes                                      !
!!$       ! --------------------------------------------------------- !
!!$       thv_env = thv
...
!!$       qt_env  = qt
!!$
!!$       betathl = (thl_env(ksfcm+3*kdir)-thl_env(ksfcm+kdir))/(0.5_r8*(dzt(ksfcm+3*kdir)+2._r8*dzt(ksfcm+2*kdir)+dzt(ksfcm+kdir)))
!!$       betaqt = (qt_env(ksfcm+3*kdir)-qt_env(ksfcm+kdir))/(0.5_r8*(dzt(ksfcm+3*kdir)+2._r8*dzt(ksfcm+2*kdir)+dzt(ksfcm+kdir)))
!!$
!!$       thl_env(ksfcm) = thl_env(ksfcm+kdir)-betathl*0.5_r8*(dzt(ksfcm+kdir)+dzt(ksfcm))
!!$       qt_env(ksfcm) = qt_env(ksfcm+kdir)-betaqt*0.5_r8*(dzt(ksfcm+kdir)+dzt(ksfcm))
!!$       if (qt_env(ksfcm) < 0._r8) qt_env(ksfcm) = 0._r8
!!$
!!$       kstart = ksfcm + kdir
!!$       if (scalesrf) then
!!$         kstart = ksfcm
...
!!$         kt_dn = k - (1+kdir)/2
!!$
!!$         thvflxup(k)= awthvup(k) - awup(k)*thv_env(kt_up)
!!$         thlflxup(k)= awthlup(k) - awup(k)*thl_env(kt_up)
!!$         qtflxup (k)= awqtup (k) - awup(k)*qt_env (kt_up)
!!$
!!$         uflxup  (k)= awuup(k) - awup(k)*u(kt_up)
!!$         vflxup  (k)= awvup(k) - awup(k)*v(kt_up)
!!$
!!$         ! if no downdrafts, should be zero since awdn should be zero
!!$         thvflxdn(k)= awthvdn(k) - awdn(k)*thv_env(kt_dn)
!!$         thlflxdn(k)= awthldn(k) - awdn(k)*thl_env(kt_dn)
!!$         qtflxdn (k)= awqtdn (k) - awdn(k)*qt_env (kt_dn)
!!$
!!$         uflxdn  (k)= awudn(k) - awdn(k)*u(kt_dn)
!!$         vflxdn  (k)= awvdn(k) - awdn(k)*v(kt_dn)
!!$
...
!!$
  ...
  subroutine poisson_notgeneralcoords(nz,nup,lambda,poi,state)
  !**********************************************************************
  ! Set a unique (but reproduceble) seed for the kiss RNG
  ! Call Poisson deviate
  ! By Adam Herrington
  ...
  end subroutine poisson_notgeneralcoords
