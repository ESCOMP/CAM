...
!jt            start(1) = (MINLOC(abs(pos_lons-pos_scmlon),dim=1))
!jt            start(2) = (MINLOC(abs(lats    -scmlat    ),dim=1))
         end if
         hdim1_d = 1
         hdim2_d = 1
...
!jt               phys_columns(col_index)%global_col_num = 1
            else
               phys_columns(col_index) = dyn_columns(col_index)
            end if
            ! Fill in physics decomp info
            phys_columns(col_index)%coord_indicies(:)=scm_col_index
