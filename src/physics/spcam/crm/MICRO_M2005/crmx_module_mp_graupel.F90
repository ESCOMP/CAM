...
#ifdef CLUBB_CRM
   use crmx_constants_clubb, only: Lv, Ls, Cp, Rv, Rd, T_freeze_K, rho_lw, grav, EP_2 => ep
#else
  ! parameters from SAM and options from wrapper routine.
   use crmx_params, only: lcond, lsub, cp, rgas, rv
#endif /*CLUBB_CRM*/

#if (defined CRM && defined MODAL_AERO)
   use crmx_drop_activation, only: drop_activation_ghan
...
!..Lookup table dimensions
      ...
      REAL(kind=selected_real_kind(12)), DIMENSION(nbins+1):: ddx
      REAL(kind=selected_real_kind(12)), DIMENSION(nbr):: Dr, dtr
      REAL(kind=selected_real_kind(12)), DIMENSION(nbs):: Dds, dts
      REAL(kind=selected_real_kind(12)), DIMENSION(nbg):: Ddg, dtg
      REAL(kind=selected_real_kind(12)), PARAMETER, PRIVATE:: lamda_radar = 0.10         ! in meters
      REAL(kind=selected_real_kind(12)), PRIVATE:: K_w, PI5, lamda4
      COMPLEX*16, PRIVATE:: m_w_0, m_i_0
      REAL(kind=selected_real_kind(12)), DIMENSION(nbins+1), PRIVATE:: simpson
      REAL(kind=selected_real_kind(12)), DIMENSION(3), PARAMETER, PRIVATE:: basis =      &
...
!..Create bins of rain (from min diameter up to 5 mm).
      ...
      ddx(nbr+1) = 0.005d0
      do n = 2, nbr
         ddx(n) = DEXP(REAL(n-1,kind=kind(0d0))/REAL(nbr,kind=kind(0d0)) &
...
!..Create bins of snow (from min diameter up to 2 cm).
      ...
      Ddx(nbs+1) = 0.02d0
      do n = 2, nbs
         Ddx(n) = DEXP(REAL(n-1,kind=kind(0d0))/REAL(nbs,kind=kind(0d0)) &
...
!..Create bins of graupel (from min diameter up to 5 cm).
      ...
      Ddx(nbg+1) = 0.05d0
      do n = 2, nbg
         Ddx(n) = DEXP(REAL(n-1,kind=kind(0d0))/REAL(nbg,kind=kind(0d0)) &
...
!      after Ray (1972)
      ...
      IMPLICIT NONE
      REAL(kind=selected_real_kind(12)), INTENT(IN):: T,lambda
      REAL(kind=selected_real_kind(12)):: epsinf,epss,epsr,epsi
      REAL(kind=selected_real_kind(12)):: alpha,lambdas,sigma,nenner
...
!      f = frequency in GHz, range 0.01 to 3000
      ...
      IMPLICIT NONE
      REAL(kind=selected_real_kind(12)), INTENT(IN):: T,lambda
      REAL(kind=selected_real_kind(12)):: f,c,TK,B1,B2,b,deltabeta,betam,beta,theta,alfa
...
!+---+-----------------------------------------------------------------+
      ...
      REAL(kind=selected_real_kind(12)), INTENT(in):: x_g, a_geo, b_geo, fmelt, lambda,  &
                                     meltratio_outside
      REAL(kind=selected_real_kind(12)), INTENT(out):: C_back
      COMPLEX*16, INTENT(in):: m_w, m_i
      CHARACTER(len=*), INTENT(in):: mixingrule, matrix, inclusion,     &
                                     host, hostmatrix, hostinclusion

      COMPLEX*16:: m_core, m_air
      REAL(kind=selected_real_kind(12)):: D_large, D_g, rhog, x_w, xw_a, fm, fmgrenz,    &
...
!+---+-----------------------------------------------------------------+
      ...
      REAL(kind=selected_real_kind(12)), INTENT(in):: volice, volair, volwater
      ...
      REAL(kind=selected_real_kind(12)):: vol1, vol2
      COMPLEX*16:: mtmp
      INTEGER:: error
...
!+---+-----------------------------------------------------------------+
      ...
      COMPLEX*16 :: m1, m2, m3
      REAL(kind=selected_real_kind(12)) :: vol1, vol2, vol3
      ...
      elseif (inclusion .eq. 'spheroidal') then
       ...
       !bloss CALL wrf_debug(150, mp_debug)
       m_complex_maxwellgarnett=CMPLX(-999.99d0,-999.99d0, kind=kind(0.d0))
       error = 1
       return
      endif
...
!..Local variables
      ...
      REAL(kind=selected_real_kind(12)), DIMENSION(kts:kte):: ilamr, ilamg, N0_r, N0_g,ilams,n0_s

      REAL, DIMENSION(kts:kte):: ze_rain, ze_snow, ze_graupel

      REAL(kind=selected_real_kind(12)):: lamg
      REAL(kind=selected_real_kind(12)):: fmelt_s, fmelt_g
...
!..Single melting snow/graupel particle 70% meltwater on external sfc
      REAL(kind=selected_real_kind(12)), PARAMETER:: melt_outside_s = 0.7d0
      REAL(kind=selected_real_kind(12)), PARAMETER:: melt_outside_g = 0.7d0

      REAL(kind=selected_real_kind(12)):: cback, x, eta, f_d
...
!   None
!-------------------------------------------------------------------------------
    use crmx_constants_clubb, only: Lv, Ls, Cp ! Constant(s)
...
END MODULE crmx_module_mp_GRAUPEL
