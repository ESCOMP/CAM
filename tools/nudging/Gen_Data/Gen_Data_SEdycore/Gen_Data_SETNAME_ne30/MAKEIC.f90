...
!
            ...
            do k = plevi,1,-1
              k_bot  = k
              hkk    = 0.5*( pressi_i(k+1,n) - pressi_i(k,n) )/ &
                                               pressi_m(k,n)
...
!
            elseif(px .ge. pbot .and. px .le. psurf ) then
              t_new(k,n) = ( tbot*(psurf - px) + tsurf*(px - pbot) ) &
                                                         /(psurf - pbot)
!
! Elseif below analysis surface: special case - heuristic equations
!
            elseif(px .gt. psurf ) then
              ...
              x   = lapse*rdair/gravit*log( presso_m(k    ,n)/ &
                                            ps_old  (      n) )
              t_new(k,n) = tsurf*(1. + x + x*x/2. + x*x*x/6.)
!
! Should never happen
!
            elseif(px .ge. p3 ) then
...
!     print *,' plevi=',plevi,' plevip1=',plevip1,' plev=',plev,'pcol=',pcol
!     print *,' loglin=',loglin
!     print *,' t_old=',t_old(:,1)
!     print *,' pressi_m=',pressi_m(:,1)
!     print *,' pressi_i=',pressi_i(:,1)
!     print *,' presso_m=',presso_m(:,1)
!     print *,' phis_old=',phis_old(1)
!     print *,' ps_old=',ps_old(1)
!     print *,' t_new=',t_new(:,1)
...
!
      write(6,*) ' '
      write(6,*) ' '
      write(6,1000) qmax*100.,nmax,kmax
      write(6,2000) qmin*100.,nmin,kmin
      if(countneg .gt. 0) write(6,3000) countneg, pcol*plev, &
                                  float(countneg)/(pcol*plev)*100.
      if(countgt1 .gt. 0) write(6,4000) countgt1, pcol*plev,  &
                                  float(countgt1)/(pcol*plev)*100.
      write(6,*) ' '
      write(6,*) ' '
!
...
! NCLFORTSTART
      subroutine read_namelist(NAMENUMBER,I_MYTMPDIR,I_MYOUTDIR,I_INPUTDIR,             &
                               I_ESMF_interp,I_ESMF_pole,I_ESMF_clean,I_TMP_clean,      &
                               I_REF_DATE,I_CASE,I_DYCORE,I_PRECISION,I_VORT_DIV_TO_UV, &
                               I_SST_MASK,I_ICE_MASK,I_OUTPUT_PHIS,I_REGRID_ALL,        &
                               I_ADJUST_STATE_FROM_TOPO,I_MASS_FIX,I_fname_phis_output, &
...
!
      ...
      character*(*) I_ESMF_interp
      character*(*) I_ESMF_pole
      character*(*) I_ESMF_clean
...
!---------------------------Local workspace-----------------------------
      ...
      namelist /makeic_nl/  MYTMPDIR,MYOUTDIR,INPUTDIR,                       &
...
!-----------------------------------
      ...
      I_ESMF_interp           =trim(ESMF_interp)
      I_ESMF_pole             =trim(ESMF_pole)
      I_ESMF_clean            =trim(ESMF_clean)
